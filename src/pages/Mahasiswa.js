...
import CreateFormModal from "../components/mahasiswa/CreateFormModal";
import EditFormModal from "../components/mahasiswa/EditFormModal";
import DeleteModal from "../components/mahasiswa/DeleteModal";
...
const Mahasiswa = () => {
  ...
  const [optionsFakultas, setOptionsFakultas] = useState();
  ...
  const getFakultasAndProdi = async () => {
    setIsFetching(true);
    try {
      const resFakultas = await API.getAllFakultas({
        page: 1,
        limit: 999,
        idUserUniversitas: params.id,
        order: "created_at desc",
      });
      const resProdi = await API.getProdis({
        page: 1,
        limit: 999,
        idUserUniversitas: params.id,
        order: "created_at desc",
      });
      setOptionsFakultas(resFakultas.data.data);
      setOptionsProdi(resProdi.data.data);
    } catch (error) {}
    setIsFetching(false);
  };
    ...
    <div className="bg-white rounded-lg p-3 sm:p-5">
      {action === "create" && (
        <CreateFormModal
          ...
          isOpen={isOpen}
          onClose={onClose}
          optionsFakultas={optionsFakultas}
          optionsProdi={optionsProdi}
              ...
              >
                ...
                <div className="flex flex-col justify-center">
                  <div class="flex items-center">
                    ...
                    {val.mahasiswa.statusMahasiswa === "alumni" && (
                      <div class="ml-2 text-xs rounded-full bg-blue-500 px-2 font-thin text-white">
                        {val.mahasiswa.statusMahasiswa.toUpperCase()}
                      </div>
                    )}
                  </div>
